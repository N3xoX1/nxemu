...
uint64_t NSP::GetProgramTitleID() const
{
    ...
    const auto iter = std::find_if(ids.begin(), ids.end(), [](uint64_t tid) { return (tid & 0x800) == 0; });
        ...
        for (const auto & inner_file : section0->GetFiles())
        {
            ...
            for (const auto & rec : cnmt.GetContentRecords())
            {
                ...
                if (next_file == nullptr)
                {
                    ...
                    {
                        LOG_WARNING(Service_FS, "NCA with ID {}.nca is listed in content metadata, but cannot be found in PFS. NSP appears to be corrupted.", id_string);
                    }
